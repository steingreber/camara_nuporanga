...
'NOME DO ARQUIVO...: pm_SUBCATEG_opt.asp
'CRIADO EM.........: 22/2/2007 12:31:50
'-------------------------------------------------------------------------------
...
If acao = "1" Then
   sNome = "NOVO REGISTRO - SUB-CATEGORIAS"
ElseIf acao = "2" Then
   sNome = "EDITAR REGISTRO - SUB-CATEGORIAS"
End If
%>
<!--#include file="_cnx.asp"-->
<html>
<head>
<title>SUBCATEG - PageMaster 2.0 :::.</title>
...
if (theForm.cp_A02TITULO && !EW_hasValue(theForm.cp_A02TITULO, "TEXT" )) {
        if (!EW_onError(theForm, theForm.cp_A02TITULO, "TEXT", "Informe um valor para o campo TITULO"))
            return false;
    }
if (theForm.cp_A01CATEG && !EW_hasValue(theForm.cp_A01CATEG, "SELECT" )) {
        if (!EW_onError(theForm, theForm.cp_A01CATEG, "SELECT", "Selecione um valor para o campo CATEGORIA"))
            return false;
    }
...
<table cellpadding="0" cellspacing="0" width="610" align="center">
    ...
    <tr>
        <td" align="center" valign="top">
            <table width="610" border="1" cellspacing="0" align="center" bordercolorlight="#000000" bordercolordark="#FFFFFF" bgcolor="#FFFFFF">
                <tr>
                    <td>
<%
Dim sSel, sNot
If acao = "1" Then %>
                        ...
                        <form action="pm_SUBCATEG_opt.asp?acao=4&tp=1&categ=<%= sCateg %>&per=<%= sPer %>" method="post" name="frmInc" enctype="multipart/form-data" id="frmInc" onSubmit="return Validator(this);">
...
<TR>
       <TD><P><span class="aFontePT">TITULO<br></span><INPUT value="" class=ud_caixa style="WIDTH:360px" name=cp_A02TITULO></P></TD>
       <TD><P><span class="aFontePT">CATEGORIA<br></span>
           <select name="cp_A01CATEG" size="1" class="ud_caixa">
<%
sCateg = Request("categ")
sLin = "Select * From CATEGORIAS Where A00IDC1="&sCateg
Set objRS = Server.CreateObject("ADODB.Recordset")
objRS.Open sLin, objConect, 3, 3
%>
                <option>Selecione</option>
<%Do While not objRs.EOF%>
                <option value="<%= objRs("A00IDC1") %>"<% If Trim(objRs("A00IDC1"))=sCateg Then %> selected<% End If %>><%= objRs("A01CATEG1") %></option>
<% objRs.MoveNext
Loop
objRs.close %>
                </select>
...
<TR>
       <td colspan="2"><P><span class="aFontePT">EXIBIR ESTE ITEM?&nbsp;</span>
                    <input type="radio" name="cp_A03EXIBIR" value="1" checked><span class="fontTD">SIM</span>&nbsp;
...
sSel = "Select * From SUBCATEG Where A00IDSUB =" & sNot
Set objRS = Server.CreateObject("ADODB.Recordset")
...
%>
                            <table align="center" border=1 cellspacing=0 width="100%" bordercolordark="white" bordercolorlight="#808080">
                        <form action="pm_SUBCATEG_opt.asp?acao=4&tp=2&sF=<% = sNot %>&categ=<%= sCateg %>&per=<%= sPer %>" method="post" name="frmInc" enctype="multipart/form-data" id="frmInc" onSubmit="return Validator(this);">
                               <tr>
                                    <td colspan="2">
                                        <input type="button" name="news_add" value=".::Cancelar::." class="botao" onClick="go2url()">&nbsp;<input type="submit" name="frmId" class="botao" value=".::Gravar::." height=19></td>
                                </tr>
<TR>
       <TD><P><span class="aFontePT">TITULO<br></span><INPUT class=ud_caixa style="WIDTH:360px" value='<% = va_A02TITULO %>' name=cp_A02TITULO></P></TD>
       <TD><P><span class="aFontePT">CATEGORIA<br></span>
           <select name="cp_A01CATEG" size="1" class="ud_caixa">
<%
sCateg = Request("categ")
...
<%Do While not objRs.EOF%>
                <option value="<%= objRs("A00IDC1") %>"<% If Trim(objRs("A00IDC1"))= va_A01CATEG Then%> selected<% End If %>><%= objRs("A01CATEG1") %></option>
<% objRs.MoveNext
Loop
objRs.close %>
                </select>
...
<% ElseIf acao = "3" Then
sNot = request.querystring("sF")
sSel = "Delete From SUBCATEG Where A00IDSUB =" & sNot
Set objRS = objConect.Execute(sSel)
response.redirect "pm_SUBCATEG.asp"
ElseIf acao = "4" Then
sNot = request.querystring("sF")
Response.Expires = 0
...
If sTipo = "1" Then
sNot = request.querystring("sF")
   sSel = sSel & "INSERT INTO SUBCATEG(A02TITULO, A01CATEG, A03EXIBIR)"
   sSel = sSel & "VALUES ('" & UCase(va_A02TITULO) & "', '" & va_A01CATEG & "', '" & va_A03EXIBIR & "')"
   Set objRS = objConect.Execute(sSel)

   response.redirect ("pm_MATERIAL_opt.asp?acao=1&categ="&sCateg&"&per="&sPer&"")

ElseIf sTipo = "2" Then

sNot = request.querystring("sF")
   sSel = sSel & "UPDATE SUBCATEG SET "
   sSel = sSel & "A02TITULO = '" & UCase(va_A02TITULO) & "', A01CATEG = '" & va_A01CATEG & "', A03EXIBIR = '" & va_A03EXIBIR & "' "
   sSel = sSel & "WHERE A00IDSUB = " & sNot
   Set objRS = objConect.Execute(sSel)
   response.redirect ("pm_MATERIAL_opt.asp?acao=1&categ="&sCateg&"&per="&sPer&"")
Else
   sSel = sSel & "UPDATE SUBCATEG SET "
...
End If
End If
'response.write "<a href=pm_MATERIAL_opt.asp?acao=1&categ="& sCateg &"&per="& sPer &">ir</a>"
