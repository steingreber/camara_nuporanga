...
SQLCateg = "Select * FROM CATEGORIAS WHERE A00IDC1 ="& sCateg
	Set objRSct = objConect.Execute(SQLCateg)
	sTitulo = objRSct(1)

SQLPass = "Select * From login Where a02email = '" & Session("usuario") & "'"
	Set objRS = objConect.Execute(SQLPass)
	If Mid(objRS("a05permissoes"),sPer,1) <> "1" Then
...
If Trim(sValor) = "" Then
   SQL = "Select * From [MATERIAL] WHERE A05CATEG ="& sCateg &" Order By [A00ID] Desc"
   iLabel = "Buscar registro"
Else
   If Trim(sTipo) = "Like" Then
   SQL = "Select * From [MATERIAL] Where " & sCampo & " " & sTipo & "'%" & sValor & "%'"
   Else
   SQL = "Select * From [MATERIAL] Where " & sCampo & " " & sTipo & "'" & sValor & "'"
   End If
   iLabel = "Exibir todos..."
...
<table cellpadding=0 cellspacing=0 width="98%" align=center>
    ...
    <tr>
		...
		&nbsp;<span class="aFonte">&raquo;&raquo;&nbsp;<%= sTitulo %>&nbsp;&laquo;&laquo;</span>
        </td>
    </tr>
    <tr>
          <form name="frmBusca" method="post" action=pm_MATERIAL.asp>
		  <input type="hidden" name="Categ" value="<%= sCateg %>">
		  <input type="hidden" name="Per" value="<%= sPer %>">
    <td height="34" align="center" colspan="3" class="tabelatitulo">
               <p><select name="q" size=1 class="ud_caixa">
               <option value="[A04DATA]">DATA</option>
               <option value="[A01TITULO]">TITULO</option>
               <option value="[A05CATEG]">CATEGORIA</option>
               <option value="[A06BUSCA]">CAMPO DE BUSCA NO SITE</option>
               </select> 
    ...
    <tr>
        <td width=494 height=20 align="center" class="tabelatitulo">
            <p><a href="pm_MATERIAL_opt.asp?acao=1&categ=<%= sCateg %>&per=<%= sPer %>"><span class="aFonte">NOVO REGISTRO</span></a></p>
        </td>
...
<span class="aFonte"><B>
<%
  Response.Write "<A HREF=""pm_MATERIAL.asp?Page=1""><img src='images/p2.gif' alt=Primeiro width=16 height=16 border=0> </A>"
if Current_Page > 1 then
  Response.Write "<A HREF=""pm_MATERIAL.asp?Page=" & (Current_Page - 1) & """><img src='images/p1.gif' alt=Anterior width=16 height=16 border=0> </A> " & vbCrLfelse
  Response.Write ""
end if
if Current_Page < Page_Count then
  Response.Write "<A HREF=""pm_MATERIAL.asp?Page=" & (Current_Page + 1) & """><img src='images/p3.gif' alt=Próximo width=16 height=16 border=0> </A>"
else
  Response.Write ""
end if
  Response.Write "<A HREF=""pm_MATERIAL.asp?Page=" & Page_Count & """><img src='images/p4.gif' alt=Último width=16 height=16 border=0>   </A>"
%>
</B></FONT></td>
    </tr>
    <tr>
                            ...
                            <tr bgcolor="<%= pmCorSel %>">
                                <td width="63">
                                    <table cellpadding="0" cellspacing="0" width="100%" align="center">
                                        <tr>
                                            <td align="center">
                                                <p><a href="pm_MATERIAL_opt.asp?acao=2&sF=<%= rsTemp("A00ID") %>&categ=<%= sCateg %>&per=<%= sPer %>"><img src="images/botao_editar.gif" alt='EDITAR' width=16 height=16 border=0></a></p>
                                            </td>
                                            <td align="center">
                                                <p><a onclick="return confirm_delete()" href=pm_MATERIAL_opt.asp?acao=3&sF=<%= rsTemp("A00ID") %>&categ=<%= sCateg %>&per=<%= sPer %>><img src="images/botao_apagar.gif" alt='EXCLUIR' width=16 height=16 border=0></a></p>
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                                <td align="Center">
                                    <p><font size="2" face="'Trebuchet MS', Verdana, Arial"><%= rsTemp("A04DATA") %></font></p>
                                </td>
                                <td align="Left">
                                    <p><font size="2" face="'Trebuchet MS', Verdana, Arial"><%= Left(rsTemp("A01TITULO"),80) %></font></p>
                                </td>
                                <td align="Center">
                                    <p><font size="2" face="'Trebuchet MS', Verdana, Arial"><%= rsTemp("A05CATEG") %></font></p>
                                </td>
                            </tr>
<%
   rsTemp.MoveNext
