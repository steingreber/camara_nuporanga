...
'NOME DO ARQUIVO...: pm_clima_opt.asp
'CRIADO EM.........: 12/1/2006 07:48:34
'-------------------------------------------------------------------------------
...
If acao = "1" Then
   sNome = "NOVO REGISTRO - ADMINISTRAÇÃO DO MUNICÍPIO"
ElseIf acao = "2" Then
   sNome = "INSIRA ARQUIVO CLIMA TEMPO NO SEU SITE."
End If
%>
...
If Session("xUser") <> True And Session("xPass") <> True Then: response.redirect "vazou.asp"
	...
	If Mid(objRS("a05permissoes"),25,1) <> "1" Then
...
<table cellpadding="0" cellspacing="0" width="610" align="center">
    ...
    <tr>
        <td" align="center" valign="top">
            <table align="center" border="1" cellspacing="0" width="610" bordercolordark="white" bordercolordark="#FFFFFF">
                <tr>
                    <td>
<%
Dim sSel, sNot
If acao = "1" Then %>
                        ...
                        <form action="pm_clima_opt.asp?acao=4&tp=1" method="post" name="frmInc" enctype="multipart/form-data" id="frmInc" onSubmit="return Validator(this);">
       ...
       <TD><P><span class="aFontePT">DESCRIÇÃO<br></span><INPUT class=ud_caixa style="WIDTH:360px" name=cp_a02desc></P></TD>
</TR>
<TR>
                        </form>    
                            </table>
...
sSel = "Select * From clima Where a00id =" & sNot
...
%>
                            <table align="center" border=1 cellspacing=0 width="100%" bordercolordark="white" bordercolordark="#FFFFFF">
                        <form action="pm_clima_opt.asp?acao=4&tp=2&sF=<% = sNot %>" method="post" name="frmInc" enctype="multipart/form-data" id="frmInc" onSubmit="return Validator(this);">
                               <tr>
                                    <td colspan="2">
                                        <input type="button" name="news_add" value=".::Cancelar::." class="botao" onClick="go2url()">&nbsp;<input type="submit" name="frmId" class="botao" value=".::Gravar::." height=19></td>
                                </tr>
                                <tr>
       <TD>
	   <span class="aFontePT">PARA ISSO DEVE ENTRAR NO SITE <a href="http://www4.climatempo.com.br/ct/temponoseusite.php#WIN" target="_blank">CLIMA TEMPO</a>, VOCÊ DEVE PRIMEIRO ESCOLHER O ESTADO E DEPOIS A CIDADE. E ENTÃO PREENCHA OS OUTROS CAMPOS DO CADASTRO CORRETAMENTE. VAI SER GERADO UM SCRIPT QUE VC DEVE COPIAR E COLAR NA ÁREA ABAIXO E CLICAR EM GRAVAR.<br></span><TEXTAREA class=ud_caixa style="width:100%; height:100;" name=cp_a01texto><% = va_a01texto %></textarea>
	   </TD>
</TR>
<TR>
                        </form>    
...
<% ElseIf acao = "3" Then
sNot = request.querystring("sF")
sSel = "Delete From clima Where a00id =" & sNot
Set objRS = objConect.Execute(sSel)
response.redirect "pm_clima.asp"
...
va_a02desc    = Replace(va_a02desc,"'",chr(34),1)

If sTipo = "1" Then
sNot = request.querystring("sF")
   sSel = sSel & "INSERT INTO clima(a01texto, a02desc)"
   sSel = sSel & "VALUES ('" & va_a01texto & "', '" & va_a02desc & "')"
   Set objRS = objConect.Execute(sSel)
